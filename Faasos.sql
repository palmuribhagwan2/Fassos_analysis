...
--3)how many succesful order delivered by each driver
select driver_id, count (distinct order_id) 
from driver_order 
where cancellation not in ('Cancellation','Customer Cancellation')
group by driver_id 

--4) how many each of type of roll was delivered
-- so we clean data (cancellation column)
...
group by customer_id,roll_id) a inner join rolls b on a.roll_id=b.roll_id
 ...
 --6) What was the maximum number of rolls delivered in single order(successive order only so we filter first)?
select * from customer_orders
select * from driver_order
...
--order_id 4 there are 3 roll_ids 
